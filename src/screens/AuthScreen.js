...
export default function AuthScreen({ navigation }) {
    ...
    const authenticate = async () => {
        ...
        if (!hasHardware && !isEnrolled) {
            Alert.alert('Erro', 'Autenticação não disponível. Por favor, configure a autenticação em seu dispositivo.');
            return;
        }
        ...
        try {
            ...
            if (result.success) {
                ...
                Alert.alert('Erro', 'Autenticação cancelada. É necessário autenticar-se.', [{ text: 'OK', onPress: () => authenticate() },], { cancelable: false });
            }
        } catch (error) {
